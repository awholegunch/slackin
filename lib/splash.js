...
const user_online_single = ' user online now of '
const user_online_plural = ' users online now of '
const user_message_single = ' user is currently registered.'
const user_message_plural = ' users are currently registered.'
...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe }){
  let div = dom('.splash',
    ...
    dom('p',
      'Join ',
      dom(`a href=https://${org}.slack.com target=_top`, name),
    ...
    dom('p.status',
      active
        ? [
          dom('b.active', active), `active === 1 ? user_online_single : user_online_plural;`,
          dom('b.total', total), ' registered.'
        ]
        : [dom('b.total', total), `total === 1 ? user_message_single : user_message_plural;`]
    ),
    dom('form id=invite',
      channels && (
        channels.length > 1
      ...
      dom('button.loading', 'Receive An Invite')
    ),
    dom('p.signin',
      'or sign in to ',
      dom(`a href=https://${org}.slack.com target=_top`, name),
